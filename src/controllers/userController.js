...
const postMessage = async function (req, res) {
  try{
    ...
    // Return a different error message in both these cases
    // let token = req.headers["x-auth-token"]
    // if(!token) return res.send({status: false, message: "token must be present in the request header"})
    // let decodedToken = jwt.verify(token, 'functionup-radon')

    // if(!decodedToken) return res.send({status: false, message:"token is not valid"})
    ...
    // if(userToBeModified != userLoggedIn) return res.send({status: false, message: 'User logged is not allowed to modify the requested users data'})
...
const deleteUser=async function(req,res){
  ...
  res.status(200).send({ status: true , message: deleteUser })
  }
catch(err){
  res.status(500).send({status: false , error: err.message})}
}
